    ...
    def draw(self, polygons, active_rotations):
        ...
        for i, face in enumerate(polygons):
            if self.texture_ids[i]:
                ...
                GL.glLineWidth(1)  # 较粗的线条
                GL.glDisable(GL.GL_TEXTURE_2D)
                GL.glColor3fv((0, 0, 0))  # Set to white
        ...
        GL.glEnable(GL.GL_LINE_SMOOTH)
        GL.glLineWidth(3)
        GL.glColor3fv((0, 0, 0))  # Set edge color to white (normalized)
...
def main():
    ...
    # Function to compute rotation matrix from axis and angle
    ...
    def matrix_multiply(a, b):
        ...
        for i in range(3):
            row = []
            for j in range(3):
                element = sum(a[i][k] * b[k][j] for k in range(3))
                row.append(element)
            result.append(row)
        return result
                    ...
                    if not active_rotations:
                        if event.key in rotate_slc_outer:
                            ...
                            for ident1, ident2 in axes_pairs:
                                try:
                                    # Use current positions to find cubes
                                    cube1 = rubik.cubes_dict[ident1]
                                    cube2 = rubik.cubes_dict[ident2]
        ...
        if active_rotations:
            ...
            for rotation in active_rotations:
                ...
                if abs(rotation['current_angle']) >= rotation['target']:
                    ...
                    # Update the cubes_dict with new positions
                    rubik.cubes_dict = {(cube.current[0], cube.current[1], cube.current[2]): cube for cube in rubik.cubes}
        ...
        for cube in rubik.cubes:
            if cube.self_rotation is not None:
                ...
                if cube.self_rotation['current_angle'] >= cube.self_rotation['target_angle']:
                    ...
                    # Apply the final rotation to the cube's rotation matrix
                    axis = cube.self_rotation['axis']
                    angle_degrees = cube.self_rotation['target_angle']
                    angle_radians = math.radians(angle_degrees)
                    rot_matrix = rotation_matrix(axis, angle_radians)
                    # Update cube's rotation matrix
                    cube.rot = matrix_multiply(rot_matrix, cube.rot)
                    # Clear self_rotation
